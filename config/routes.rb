Rails.application.routes.draw do
  get 'clear_business' => 'common_actions#clear_business'
  get 'find_business' => 'common_actions#find_business'
  get 'documents' => 'common_actions#documents'
  get 'declarations' => 'common_actions#declarations'
  get 'new_vehicle', to: 'vehicles#new', as: 'new_vehicle'
  post 'create_vehicle', to: 'vehicles#create'
  get '/vehicle/:id', to: 'vehicles#show', as: 'vehicle'
  get '/edit_vehicle/:id', to: 'vehicles#edit', as: 'edit_vehicle'
  patch '/update_vehicle/:id', to: 'vehicles#update', as: 'update_vehicle'
  delete '/delete_vehicle/:id', to: 'vehicles#destroy', as: 'delete_vehicle'
  get 'new_estate', to: 'estates#new', as: 'new_estate'
  post 'create_estate', to: 'estates#create', as: 'create_estate'
  get '/estate/:id', to: 'estates#show', as: 'estate'
  get '/edit_estate/:id', to: 'estates#edit', as: 'edit_estate'
  patch '/update_estate/:id', to: 'estates#update', as: 'update_estate'
  delete '/delete_estate/:id', to: 'estates#destroy', as: 'delete_estate'
  get 'index' => 'inspector_actions#index'
  get '/list/:field' => 'inspector_actions#list', as: 'list'
  get '/show_user/:id', to: 'inspector_actions#show_user', as: 'show_user'
  get '/new_notification/:id/:receiver', to: 'notifications#new', as: 'new_notification'
  post '/create_notification/:user_id/:receiver', to: 'notifications#create', as: 'create_notification'
  get '/notification/:id', to: 'notifications#show', as: 'notification'
  delete '/delete_notification/:id', to: 'notifications#destroy', as: 'delete_notification'
  get '/new_ownership/:object_type/:object_id', to: 'ownerships#new_for_vehicle_or_estate', as: 'new_ownership'
  get '/new_ownership_for_income/:object_type/:object_id', to: 'ownerships#new_for_income', as: 'new_ownership_for_income'
  post '/create_ownership/:object_type/:object_id', to: 'ownerships#create', as: 'create_ownership'
  get '/ownership/:id', to: 'ownerships#show', as: 'ownership'
  delete '/delete_ownership/:id', to: 'ownerships#destroy', as: 'delete_ownership'
  root 'main_page#home'
  get 'sessions/new'
  get 'account' => 'personal_actions#account'
  get 'become_entrepreneur' => 'personal_actions#become_entrepreneur'
  get 'create_business' => 'personal_actions#create_business'
  patch 'confirm_status' => 'personal_actions#confirm_status'
  get 'fill_balance' => 'personal_actions#fill_balance'
  patch 'update_balance' => 'personal_actions#update_balance'
  get 'send_files' => 'personal_actions#send_files'
  post 'attach_files' => 'personal_actions#attach_files'
  get 'property' => 'personal_actions#property'
  get 'pay_taxes' => 'personal_actions#pay_taxes'
  get '/signup/:field', to: 'users#new', as: 'signup'
  post '/create_user/:user_id', to: 'users#create', as: 'create_user'
  get 'show' => 'users#show'
  get 'login' => 'sessions#new'
  post '/create_session/:field', to: 'sessions#create', as: 'create_session'
  delete 'logout' => 'sessions#destroy'
  get '/auth_choice/:field' => 'sessions#auth_choice', as: 'auth_choice'
  get '/edit/:id', to: 'users#edit', as: 'edit'
  patch '/update/:id', to: 'users#update', as: 'update'
  delete '/delete_user/:id', to: 'users#destroy', as: 'delete_user'

  resources :users

  # Define your application routes per the DSL in https://guides.rubyonrails.org/routing.html
  # Defines the root path route ("/")
  # root "articles#index"
end
